...
public class PoolManager : MonoBehaviour
{
    ...
    /// </summary>
    public static PoolManager x = null;
    ...
    /// The array of pools
    /// </summary>
    [SerializeField] private Pool[] pA_pools;
    ...
    private void InitializeAllObjects()
    {
        for (int i = 0; i < pA_pools.Length; i++)
        {
            ...
            for (int j = 0; j < pA_pools[i].i_Size; j++)
            {
                ...
                pA_pools[i].AddAtIndex(j, NewObj.GetComponent<IPoolable>());
                NewObj.GetComponent<IPoolable>().SetInPool(true);
                NewObj.GetComponent<IPoolable>().SetIndex(j);
    ...
    private void AddNewToPoolAndSpawn(GameObject _go_objectToSpawn, Vector3 _v_spawnPoint, Quaternion _q_rotation, bool hasAKey)
    {
        ...
        if (!hasAKey)
        {
            ...
            Pool[] newPoolArr = new Pool[newSize];
            for (int i = 0; i < pA_pools.Length; i++)
            {
        ...
        pA_pools[D_poolIndexes[_go_objectToSpawn.name.Replace("(Clone)", "")]].ResizeArray(_go_objectToSpawn.GetComponent<IPoolable>());
    ...
    public GameObject SpawnNewObject(GameObject _go_objectToSpawn, Vector3 _v_spawnPoint, Quaternion _q_rotation)
    {
        ...
        else
        {
            ...
            if (_go_objectToSpawn.GetComponent<IPoolable>() != null)
            {
                GameObject newOb = Instantiate(_go_objectToSpawn);
                newOb.name = newOb.name.Replace("(Clone)", "");
                AddNewToPoolAndSpawn(newOb, _v_spawnPoint, _q_rotation, hasKey);
